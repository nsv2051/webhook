...
# 登录密码
passwd1 = "密码"
# 酷推skey和server酱sckey只用填一个另一个留空即可
skey = "2c0e38305b17549bde7dfa29"
# 推送server酱
sckey = ""
# 要修改的步数，直接输入想要修改的步数值，（默认）留空为随机步数，改了这个直接运行固定值（用于测试）
step1 = "3067"
# 开启根据地区天气情况降低步数（默认关闭）
open_get_weather = True
# 地区（上面开启后必填）如：area = "宁波"
area = "郑州"
# 运行程序时间点24小时制（不要设置0，1，2），这边设置好云函数触发里也要改成相同的小时运行，如：30 0 9,11,14,16,18,20,22 * * * *
time_list = [8, 10, 13, 15, 17, 19, 21]
# 设置运行结果推送不推送与上面时间一一对应（当前表示为21点推送其余时间运行不推送结果）
set_push = [True, True, True, True, True, True, True]
# 最小步数（如果只需要刷步的次数少于7次就将该次数以后的步数改成0，如：time_list[3]: 0，表示第五次开始不运行或者直接云函数触发里面不在该时间调用即可）
min_dict = {time_list[0]: 1500, time_list[1]: 3000, time_list[2]: 5000, time_list[3]: 8000, time_list[4]: 11000, time_list[5]: 15000, time_list[6]: 19000}
# 最大步数（例如现在设置意思是在8点运行会在1500到2999中随机生成一个数提交（开启气候降低步数会乘系数K）10点3000~4999。。。以此类推）
max_dict = {time_list[0]: 2999, time_list[1]: 4999, time_list[2]: 7999, time_list[3]: 10999, time_list[4]: 14999, time_list[5]: 18999, time_list[6]: 22999}
